...
def list_interactions(index, matrix, res2):
    ...
    for i in row_sorted:
        ind = row.index(i)
        if i < 0:
            list_interact.append(res2[ind] + '(+)')
        if i > 0:
            list_interact.append(res2[ind] + '(-)')
    return (list_interact)

def value_interactions(index, matrix, res2):
    value_interact = []
    row = matrix.loc[index , : ]
    row = row.tolist()
    row_sorted = sorted(row, reverse=False)
    for i in row_sorted:
        if i != 0:
            value_interact.append(i)
    return (value_interact)
...
def main():
    ...
    for residue in res1:
        ...
        values_interact = value_interactions(residue, matrix, res2)
        surface_list.append(str(list_interact)[1:-1])
        values_list.append(str(values_interact)[1:-1])
        if len(list_interact) > 0:
            print (residue + ': ' + str(list_interact)[1:-1])
    
    matrix['Interactions'] = surface_list
    matrix['Values'] = values_list
    
    matrix[['Interactions', 'Values']].to_csv(args.output_name)
